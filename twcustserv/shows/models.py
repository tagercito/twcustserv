...
class AuditedModel(models.Model):
    ...
    display_text = models.CharField(_('display text'), max_length=254,
                                    blank=True)
    admin_name = models.CharField(_('Admin unicode'), max_length=254,
    ...
    def field_has_changed(self, field):
        from_db_obj = self.__class__._default_manager.get(pk=self.pk)
        old_value = getattr(from_db_obj, field.name)
        try:
...
class Show(AuditedModel):
   ...
   # mail_content = models.ForeignKey(Content, null=True, blank=True, related_name='mail_show')
    ...
    cms_url = models.CharField('URL redirect CMS', max_length=255, null=True,
                               blank=True)
   ...
   # insurance = models.ForeignKey('insurance.Insurance', null=True, related_name = 'insurance')
    ...
    #objects = ShowManager()
    extra_tickets = models.BooleanField(_('Seleccionar mas entradas'),
                                        default=False)
...
class Performance(AuditedModel):
    ...
    venue = models.ForeignKey('Venue', related_name='venues', null=True,
                              blank=True)
    venue_profile = models.ForeignKey('VenueProfile', null=True, blank=True,
                                      on_delete=models.SET_NULL)
    shipping_points = models.ManyToManyField('ShippingPoint',
                                             related_name='performances',
                                             through='shows.ShippingPointDetails')

    is_series = models.BooleanField(_('is this performance a series?'),
                                    default=False)
    seat_selection_disabled = models.BooleanField(_('seat selection disabled'),
                                                  default=False)
    # Basic
    perf_code = models.CharField(_('Code'), max_length=32)
    event_name = models.CharField(_('Event Name'), max_length=32, null=True,
                                  blank=True)
    ...
    when = models.CharField(_('When'), max_length=100, blank=True,
                            help_text=_(
                                'The content of this field will be'
    ...
    is_active = models.BooleanField(_('Is Active?'), default=True)
    #credit_cards = models.ManyToManyField('purchase.Card',
                                          #related_name='performances')
    can_print_at_home = models.BooleanField(_('Can print at home?'),
                                            default=False)
    #insurance = models.ForeignKey('insurance.Insurance',
                                  #null=True, blank=True,
                                  #related_name = 'perf_insurance')
    use_local_seatmap = models.BooleanField(
        default=False,
        help_text='Poner en True para no pedir a softix el '
        'seatmap en cada proceso de compra')
    order = models.PositiveIntegerField(_('Orden Para Mostrar'), default=0)
    sales_start = models.DateTimeField(_('Start date of sales'), null=True,
                                       blank=True)
    sales_end = models.DateTimeField(_('End date of sales'), null=True,
                                     blank=True)
    #Specific
    max_qty = models.IntegerField(default=6)
    la_nacion_quota = models.PositiveIntegerField(_('Cupo para La Nacion'),
                                                  default=0)
    extra_tickets = models.BooleanField(_('Seleccionar mas entradas'),
                                        default=False)
    social_share = models.BooleanField(_('Habilitar la compra social?'),
                                       default=False)
    ...
    def __str__(self):
        try:
            return '{}/{}'.format(self.show, self.perf_code)
        except:
            return ''
...
class Series(AuditedModel):
    ...
    def __str__(self):
        try:
            return '{}/{}'.format(self.desc, self.performance)
        except:
            return ''
...
class PriceType(AuditedModel):
    ...
    #mail_content = models.ForeignKey(Content, null=True, blank=True, 
                                     #elated_name='mail_price_type')
    #benefit = models.ForeignKey('benefits.Benefit', null=True, blank = True)

    # Basic
    name = models.CharField(_('Name'), max_length=32)
    code = models.CharField(_('Code'), max_length=1)
    dayos_code = models.CharField(_('Dayos Code'), max_length=1, null=True,
                                  blank=True)
    ...
    def __str__(self):
        try:
            return '{}/{}'.format(self.performance, self.code)
        except:
            return ''
...
class PriceCategory(AuditedModel):
    ...
    full_price = models.DecimalField(
        _('Full Price'), max_digits=8, decimal_places=2, default=-1,
        help_text=_('The price plus the discount. -1 If no discount'))
    max_qty = models.IntegerField(_('max qty'),  null=True, blank=True,
                                  default=0)
    hidden = models.BooleanField(default=False)
    is_active = models.BooleanField(_('Is Active?'), default=True)
    ...
    def __str__(self):
        try:
            return '{}/{}'.format(self.price_type.__unicode__(), self.section.section)
        except:
            return ''
...
class Section(AuditedModel):
    ...
    show_seat_description = models.BooleanField(
        _('Mostrar Fila/Asiento en resumen de'), default=True)
    ...
    def __str__(self):
        try:
            return '{}/{}'.format(self.performance, self.section)
        except:
            return ''
...
class ShippingPointDetails(AuditedModel):
    ...
    performance = models.ForeignKey('Performance',
                                    related_name ='shipping_point_detail')
    shipping_point = models.ForeignKey('ShippingPoint',
                                       related_name ='shipping_point_detail')
    #content = models.ForeignKey(Content, null=True, blank = True)
    ...
    def __str__(self):
        try:
            return '{}/{}'.format(self.performance, self.shipping_point)
        except:
            return ''
...
class VenueShippingPointDetails(AuditedModel):
    venue = models.ForeignKey('Venue',
                              related_name ='venue_shipping_point_detail')
    shipping_point = models.ForeignKey('ShippingPoint',
                                       related_name ='venue_shipping_point_detail')
    #content = models.ForeignKey(Content, null=True, blank = True)
    ...
    def __str__(self):
        try:
            return '{}/{}'.format(self.venue, self.shipping_point)
        except:
            return ''
...
class Venue(Location):
    ...
    shipping_points = models.ManyToManyField(
        'ShippingPoint',
        related_name='venue_shipping_points',
        through='shows.VenueShippingPointDetails')
    ...
    def __str__(self):
        try:
            return '{} - {}'.format(self.code, self.display_text)
        except:
            return ''
...
class VenueProfile(AuditedModel):
    ...
    #background = models.ImageField(upload_to='venueprofile', null=True,
                                   #blank=True)
    default = models.BooleanField(default=False)
    small_section = models.BooleanField(default=False)
    show_seat_description = models.BooleanField(
        _('Mostrar Fila/Asiento en resumen de'),
        default=True)
    ...
    def get_section_group_name(self, color):
        try:
            vp_colorgroup = VenueProfileColorGroup.objects.get(
                venue_profile=self, color=color)
    ...
    def __str__(self):
        try:
            return '{} - {} profile'.format(self.venue, self.display_text)
        except:
            return ''
...
def update_default(sender, instance, created, **kwargs):
    if instance.default:
        q = VenueProfile.objects.filter(
            default=True, venue=instance.venue).exclude(id=instance.id)
...
class Area(models.Model):
    ...
    venue_profile_section = models.ForeignKey('VenueProfileSection',
                                              related_name='areas')
    code = models.CharField(max_length=255)
...
class VenueProfileSection(AuditedModel):
    ...
    #color = RGBColorField()
    #thumbnail_image = models.ImageField(upload_to='venueprofile', null=True,
                                        #blank=True)
    thumbnail = models.URLField()
    #ticket_info_image = models.ImageField(upload_to='venue_ticketinfo',
                                          #null=True, blank=True)
    ticket_info_description = models.TextField()

    def __str__(self):
        try:
            return '{} : {}'.format(self.venue_profile, self.section)
        except:
            return ''
...
class VenueProfileSectionInfo(AuditedModel):
    ...
    section = models.ForeignKey(VenueProfileSection,
                                related_name="venue_profile_section_info")
...
class VenueProfileColorGroup(AuditedModel):
    ...
    def __str__(self):
        try:
            return '{} - {}'.format(self.name, self.venue_profile)
        except:
            return ''
    ...
    def create_shipping_point_details(self):
        for perf in self.performance.all():
            for shipping_point in self.shipping_point.all():
                try:
                    ShippingPointDetails.objects.create(
...
class ImportShowLog(models.Model):
    ...
    def __str__(self):
        try:
            return '{} - {}'.format(self.show, self.date)
        except:
            return ''
