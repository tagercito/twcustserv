...
class Account(models.Model):
    ...
    user = models.ForeignKey('auth.User', related_name='account_user',
                             null=True)
    ...
    atype = models.PositiveSmallIntegerField(_('account type'), null=True,
                                             blank=True,
                                             help_text=_('Set by Softix'))
    ...
    apartment = models.CharField(_('apartment number'), max_length=16,
                                 blank=True)
    housenumber = models.CharField(_('house number'), max_length=16,
                                   blank=True)
    comment = models.CharField(_('comment'), max_length=255, blank=True)
    email_addr = models.EmailField(_('email address'))
    site_attrib = models.BooleanField(
        _('arbitrary boolean flag - for internal usage'))
    ...
    member_since = models.DateField(_('member since'), blank=True, null=True,
                                    ...
                                    editable=False)
    primary_season = models.CharField(_('primary season'), max_length=32,
                                      blank=True)
    primary_an = models.PositiveIntegerField(_('primary account'), null=True,
                                             blank=True)
    ...
    def __str__(self):
        r = [str(data) for data in [self.account, self.full_name, self.season]
             if data]
        return ' - '.join(r)

    @property
    def full_name(self):
        n = [getattr(self, "name"+str(x)).strip() for x in range(1, 6)]
        return ' '.join(n)
